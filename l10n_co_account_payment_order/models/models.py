...
class AccountBatchPaymentExtended(models.Model):
    ...
    def _check_payments_constrains(self):
        for record in self:
            ...
            non_draft_posted = record.payment_ids.filtered(lambda p: p.state not in ('draft', 'posted'))
            if non_draft_posted:
                names = '\n'.join([p.name or _('Id: %s', p.id) for p in non_draft_posted])
                msg = _('You cannot add payments that are not draft or not posted.\nPayments:\n%s', names)
                raise ValidationError(msg)
...
class AccountPaymentOrderWizard(models.TransientModel):
    ...
    partner_id = fields.Many2one('res.partner', string='Partner', required=False,
                                 index=True, domain=[('supplier_rank', '>=', 0)])
    ...
    def _check_payments_constrains(self):
        for record in self:
            ...
            if payment_null:
                ...
                msg = _('You cannot add payments with zero amount in a Batch Payment.\nPayments:\n%s', names)
                raise ValidationError(msg)
            non_posted = record.payment_ids.filtered(lambda p: p.state != self.state)
            if non_posted:

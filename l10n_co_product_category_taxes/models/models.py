...
class ProductTemplate(models.Model):
    ...
    @api.model
    def _get_array_value(self, value):
        array = eval(value) if type(value) is str else value
    ...
    def create(self, vals):
        if vals.get('categ_id'):
            ...
            if vals.get('taxes_id'):
                vals['taxes_id'] = self._get_array_value(vals.get('taxes_id', []))
                vals['taxes_id'].extend(new_taxes['taxes_id'])
            else:
                vals['taxes_id'] = new_taxes['taxes_id']

            if vals.get('supplier_taxes_id'):
                vals['supplier_taxes_id'] = self._get_array_value(
                    vals.get('supplier_taxes_id', []))
                vals['supplier_taxes_id'].extend(new_taxes['supplier_taxes_id'])
            else:
...
class ProductCategory(models.Model):
    ...
    taxes_id = fields.Many2many(
        'account.tax', 'product_category_taxes_rel', 'category_id', 'tax_id',
    ...
    supplier_taxes_id = fields.Many2many(
        'account.tax', 'product_category_supplier_taxes_id', 'category_id',
        'tax_id', string="Vendor Taxes",
